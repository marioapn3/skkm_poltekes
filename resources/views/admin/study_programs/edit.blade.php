...
@section('title', 'Ubah Program Studi - Poltekes Yogyakarta')
@section('content')
    ...
    <div class="px-4 pt-6 rounded-lg">
        <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
            ...
            <form action="{{ route('admin.study-program.update', $studyProgram->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4 lg:pr-56 xl:pr-72 md:grid-cols-2">
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            placeholder="Masukan nama anda" value="{{ $studyProgram->name }}" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="lecture" name="head_of_study"
                            ...
                            {{-- @if ($user->student->lecture != null)
                                <option selected value="{{ $user->student->lecture_id }}">
                                    {{ $user->student->lecture->user->name }} -
                                    {{ $user->student->lecture->nip }}
                                </option>
                            @else
                                <option selected value="">Pilih Dosen</option>
                            @endif --}}
                            <option selected value="{{ $studyProgram->head_of_study }}">
                                {{ $studyProgram->headStudy->user->name }} - {{ $studyProgram->headStudy->nip }}</option>
                            @foreach ($lectures as $lecture)
                                @if ($studyProgram->head_of_study == $lecture->id)
                                    @continue
                                @endif
